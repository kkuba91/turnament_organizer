...
# Get the directory where the module is installed (not cwd)
_module_root = Path(__file__).parent.parent.parent
_templates_dir = str(_module_root / "Templates")
templates = Jinja2Templates(directory=_templates_dir)

api = FastAPI(
    title="Chess turnament manager",
...
class ApiData:
    ...
    def run_api(self):
        ...
        Start API app.
        """
        kwargs = {"app": self.api, "port": self.port, "loop": "asyncio"}
        uvicorn.run(**kwargs)
    ...
    def _init_endpoints(self):
        # Mount static files for serving JS/CSS
        _style_dir = _module_root / "Templates" / "Styles"
        self.api.mount("/static", StaticFiles(directory=str(_style_dir)), name="static")

        # App general:
        tag = self.Tags.app
        self.routers[tag] = APIRouter()
        self.routers[tag].add_api_route(
            path="/",
        ...
        self.routers[tag].add_api_route(
            path="/turnament/player/edit",
            endpoint=self.turnament_player_edit,
            methods=["POST"],
            description="Edit Player data",
            tags=[tag],
        )
        self.routers[tag].add_api_route(
            path="/turnament/player/stats",
            endpoint=self.turnament_player_stats,
            methods=["GET"],
            description="Get Player statistics",
            tags=[tag],
        )
        self.routers[tag].add_api_route(
            path="/turnament/players",
        ...
        self.routers[tag].add_api_route(
            path="/turnament/players/suspend_list",
            endpoint=self.turnament_players_suspend_list,
            methods=["GET"],
            description="Get Player list with suspend status",
            tags=[tag],
        )
        self.routers[tag].add_api_route(
            path="/turnament/players/suspend",
            endpoint=self.turnament_players_suspend,
            methods=["POST"],
            description="Set suspend status for players",
            tags=[tag],
        )
        self.routers[tag].add_api_route(
            path="/turnament/results",
        ...
        self.routers[tag] = APIRouter()
        self.routers[tag].add_api_route(
            path="/turnament/round/get_results",
        ...
        self.routers[tag].add_api_route(
            path="/turnament/round/get_results/html",
        ...
        self.routers[tag].add_api_route(
            path="/turnament/round/set_result",
        ...
        self.routers[tag].add_api_route(
            path="/turnament/round/apply_results",
    ...
    async def start_turnament(self, rounds: int, system_type: str):
        await asyncio.sleep(0.01)
        logging.info(
            "[API]: Starting actual turnament. Rounds: {}. System: {} ..".format(
                rounds, system_type
            )
        )
    ...
    async def turnament_player_add(
        self, name: str, surname: str, sex="male", city="", category="wc", elo=0
    ):
        await asyncio.sleep(0.01)
        logging.info("[API]: Add Player: {} ..".format(name))
        return self.app.actions.player_add(
            name=name, surname=surname, sex=sex, city=city, category=category, elo=elo
        )

    async def turnament_player_del(self, name: str, surname=""):
        await asyncio.sleep(0.01)
        logging.info("[API]: Remove Player: {} ..".format(name))
        return self.app.actions.player_del(name=name, surname=surname)

    async def turnament_player_edit(
        self,
        player_id: int,
        name: str,
        surname: str,
        ...
        await asyncio.sleep(0.01)
        logging.info(f"[API]: Edit Player #{player_id} ..")
        return self.app.actions.player_edit(
            player_id=player_id,
            name=name,
            surname=surname,
            sex=sex,
    ...
    async def turnament_player_stats(self, player_id: int):
        await asyncio.sleep(0.01)
        logging.info(f"[API]: Get Player #{player_id} stats ..")
        return self.app.actions.player_stats(player_id=player_id)

    async def turnament_players(self, type: str):
        await asyncio.sleep(0.01)
        logging.info(f'[API]: Get Player list for "{type}" ..')
        return self.app.actions.players_get(type=type)

    async def turnament_players_suspend_list(self):
        await asyncio.sleep(0.01)
        logging.info("[API]: Get suspend list ..")
        return self.app.actions.players_suspend_list()

    async def turnament_players_suspend(self, request: Request):
        await asyncio.sleep(0.01)
        body = await request.json()
        logging.info(f"[API]: Suspend players: {body} ..")
        return self.app.actions.players_suspend(
            suspended=body.get("suspended", []), unsuspended=body.get("unsuspended", [])
        )

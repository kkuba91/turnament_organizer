...
class Round(ModelRound):
    ...
    def add_table(self, player_w, player_b):
        ...
        table = Table(number=next_nr, w_player=player_w, b_player=player_b)
        ...
        if next_nr > 1:
            if (
                self.tables[next_nr - 1].w_player.id == -1
                or self.tables[next_nr - 1].b_player.id == -1
            ):
    ...
    def get(self, full=True):
        data = {"nr": self.number, "tables": [], "pauser": self.pausing}
        table_nr = 1
        for _ in range(1, len(self.tables) + 1):
            if table_nr not in self.tables:
                table_nr += 1
                continue
            specific = ["id", "name", "surname", "cat", "elo", "result"]
            table = self.tables[table_nr].get(full=full, specific=specific)
            table["nr"] = table_nr
            # Skip the pauser table from main display (pauser plays with fantom id=-1)
            w_id = table["white"]["id"]
            b_id = table["black"]["id"]
            if w_id == -1 or b_id == -1:
                table_nr += 1

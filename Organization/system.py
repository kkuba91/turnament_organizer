    ...
    def _set_tables(self, scored_players):
            ...
            if not _played:
                ...
                for opponent in players_list:
                    ...
                    if not opponent_found and not _oppo_played:
                        ...
                        if (
                            not opponent_found
                            and opponent._idnt not in player._opponents
                        ):
                            ...
                            players_set.append(
                                player._idnt
                            )  # Add players to 'set' list
                            players_set.append(opponent._idnt)
                            # Add table with these two players:
                            nr = _round.add_table(
                                player_w=player._idnt, player_b=opponent._idnt
                            )
        ...
        if not _paused_right:
            for player in reversed(players_list):
                if (
                    player._idnt != _round.pausing and not _replace_ok
                ):  # Not the same player
                    ...
                    if (
                        _oppo_idnt not in _paused_player._opponents
                        and not _oppo._paused
                    ):
        ...
        if not _paused_right and _replace_ok:  # DEBUG
            print(
                f"[DEBUG] Replace pausing {_replace_pausing} with {_replace_opponent}!"
            )
        ...
        if not _paused_right and _replace_ok:  # DEBUG
            print(
                f"[DEBUG] Round needs to be recalculated: {_round.pausing} cannot pause!"
            )
